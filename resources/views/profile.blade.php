...
@section('main')
    <section class="container">
        ...
        <section>
            ...
            <article>
                <h3 class="fw-bold mb-2 text-uppercase">Email</h3>
                <div class="table">
                        ...
                        <div class="row">
                            ...
                            <div class="cell">
                                <input type="button" value="Default"
                                    ...
                                    @hidden(!$email->isVerified())
                                    @class([
                                        'btn btn-primary' => !$email->is_default,
                                        'btn btn-secondary' => $email->is_default,
                                    ])
                                >
                            </div>
                            <div class="cell">
                                <input type="text" size="6" minlength="6" maxlength="6" placeholder="Verify Code"
                                    @hidden($email->isVerifing())>
                            </div>
                            <div class="cell" @hidden($email->isVerifing())>
                                <input type="button" value="Renew verify code" class= "btn btn-warning">
            ...
            <article>
                <h3 class="fw-bold mb-2 text-uppercase">Moblie</h3>
                @foreach ($user->mobiles as $mobile)
                    <div class="row g-6">
                        <div class="col-mb-2">{{ $mobile->mobile }}</div>
                        <div class="col-mb-2">
                            <span @hidden($mobile->is_default)>Default</span>
                        </div>
                        <div class="col-mb-2">
                            <input type="text" size="6" minlength="6" maxlength="6" placeholder="Verify Code"
                                data-generated-at="{{ $mobile->lastVerification->expired_at ?? '' }}"
                                id="mobileVerifyCodeInput{{ $mobile->id }}" hidden disabled>
                        </div>
                        <div class="col-md-2">
                            <button type="button" id="mobileRenewVerifyCodeButton{{ $mobile->id }}"
                                class="form-control btn btn-primary" disabled hidden onclick="return false;">
                                Send new verify code<span id="mobileRenewVerifyCodeCountDown{{ $mobile->id }}"></span>
                            </button>
                        </div>
                        <div class="col-md-2">
                            <button type="button" id="resendMobileVerifyCodeButton{{ $mobile->id }}"
                                class="form-control btn btn-primary" disabled hidden>
                                Submit<span id="mobileSubmitVerifyCountDown{{ $mobile->id }}"></span>
                            </button>
                            <button type="button" id="mobileS0ubmittingButton{{ $mobile->id }}"
                                class="form-control btn btn-primary" disabled hidden>
                                <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                Submitting...
                            </button>
                        ...
                        <div class="col-md-2">
                            <input type="button" id="mobileCancelVerifyButton{{ $mobile->id }}"
                                class="btn btn-danger form-control"
                                value="Cancel" hidden disabled>
                        </div>
                        <div class="col-md-2">
                            <input type="button" id="mobileVerifyButton{{ $mobile->id }}"
                                class="btn btn-primary form-control" value="Verify">
                        </div>
                    </div>
                @endforeach
            </article>
